...
- Programmatic single-member fetch via `GET /v1/members/{user_id}`, behind the existing least-privilege `members:read` scope. `GET /v1/members` already shipped as the pull side of the IGA contract (Okta Lifecycle, SailPoint, Workday JML pipelines), but identity-runbook scripts and SCIM reconciliation jobs that just wanted to confirm one user's current role or suspension state still had to pull and walk the entire roster, which scaled badly for workspaces with thousands of members and made per-user diff loops chatty against the rate-limit budget. The new verb returns the same member shape (user id, email, role, status, joined timestamp, plus suspension and just-in-time support grant fields) for a single id, scoped strictly to the calling key's workspace. Tenant scope is structural: the workspace is always resolved as `getWorkspace(key.workspaceId)` (no query string or body field can redirect the lookup), the target is found inside that workspace's roster, and cross-tenant user ids return 404 (not 403) so a workspace A key cannot use status codes to probe whether a userId exists in workspace B. Keys with no workspace binding receive `invalid_request` rather than falling through to an empty result. The full `/v1` enforcement chain (lockdown, workspace + per-key IP allowlists, residency, API-key policy) runs before any member is read, the per-key rate-limit window is enforced (not peeked) so this cannot be used as a free heartbeat, and every call writes a `v1.members.get` row to the tamper-evident audit chain. Documented in `/v1/discovery`, `/v1/openapi.json|yaml`, and the `members-get` spec entry. Pinned by `web/tests/v1-members-get-tenant-isolation.test.ts` (live cross-tenant isolation at the lib layer, scope rejection, spec coverage, plus a source-level wiring check that fails on regression if the scope filter, the rate-limit enforce, the tenant scope, the workspace gates, or the audit row is dropped).

  ### Try it: confirm one member's role from an IGA runbook

  ```bash
  # Mint a least-privilege key on /api-keys with only the members:read scope.

  # Look up a single member by user id (404 if not in the calling workspace):
  curl -sS http://localhost:3000/v1/members/u_91 \
    -H "Authorization: Bearer $CODECLONE_KEY"
  # {"member":{"user_id":"u_91","email":"bob@acme.com","role":"editor","status":"active",...}}
  ```
...
- Programmatic break-glass workspace lockdown via `GET/POST/DELETE /v1/lockdown`, behind the new `lockdown:read` and `lockdown:write` scopes. When a credential compromise, insider-threat alert, or active investigation requires halting every /v1 call to a workspace within seconds, SecOps needs to fire that switch from a SOAR playbook, not a human dashboard login. The `GET` endpoint returns whether the workspace is locked and (when locked) the reason, optional case reference, placed-at timestamp, and placed-by actor for SIEM correlation and SOC2 CC7.3 incident-response evidence. `POST` engages the lockdown with a validated reason (3 to 500 chars) and an optional ticket id (PagerDuty, Jira, ServiceNow); from that moment every other `/v1` route bound to the workspace refuses with HTTP 423 `workspace_locked`. `DELETE` lifts the lockdown but requires the workspace slug as a confirmation token so a misconfigured SOAR playbook cannot accidentally release the wrong tenant. Tenant scope is structural: the workspace is always `key.workspaceId`, never read from a query parameter or body field, so a key in workspace A cannot read or mutate workspace B's lockdown state. Writes additionally require that the calling key's owning user is a current owner of the workspace, mirroring the `canManage` rule that gates the dashboard editor, so a SOAR service key with `lockdown:write` cannot privilege-escalate past the human policy. This route deliberately omits the workspace lockdown enforcement gate that every other `/v1` route runs (and only this gate): otherwise placing a lockdown would soft-brick the API release path. Every other `/v1` policy still applies (workspace + per-key IP allowlist, residency, API-key policy, per-key rate-limit enforce-not-peek so the call is metered). Every call writes a `v1.lockdown.{read,place,release}` audit row with a before/after diff on writes; denied writes by non-owners are also audited so privilege-escalation attempts are visible in the SIEM. Pinned by `web/tests/v1-lockdown-tenant-isolation.test.ts` (scope split, policy chain minus the deliberate lockdown carve-out, owner gate on writes, slug-as-confirmation guard, live cross-tenant isolation between two workspaces on a shared on-disk store).
  ...
  ```bash
  # 1. Read current lockdown status for a SOC2 CC7.3 evidence collector.
  curl -sS https://codeclone.example.com/v1/lockdown \
    -H "Authorization: Bearer $CODECLONE_API_KEY"

  # 2. Engage break-glass lockdown in response to a credential-compromise alert.
  curl -sS -X POST https://codeclone.example.com/v1/lockdown \
    -H "Authorization: Bearer $CODECLONE_API_KEY" \
    -H "Content-Type: application/json" \
    -d '{"reason":"Suspected key compromise: see PD-1042.","caseRef":"PD-1042"}'

  # While active, every other /v1 endpoint bound to this workspace
  # returns HTTP 423 workspace_locked with a Retry-After header.

  # 3. Lift the lockdown after the incident. Slug confirms the right tenant.
  curl -sS -X DELETE https://codeclone.example.com/v1/lockdown \
    -H "Authorization: Bearer $CODECLONE_API_KEY" \
    -H "Content-Type: application/json" \
    -d '{"confirm":"acme"}'
  ```

- Programmatic audit-chain tamper-evidence verification via `GET /v1/audit/verify`, behind the existing `audit:read` scope. SOC2 CC7.2 and ISO 27001 A.12.4 both require evidence that audit logs are protected against undetected modification; CodeClone already writes every audit row into an append-only sha256 hash chain (seq, prevHash, hash linked to GENESIS_HASH), but until now the only way to verify the chain was the cookie-authenticated `/api/audit/verify` dashboard route, which is meant for a human compliance reviewer clicking a button. Enterprise SecOps and continuous-controls-monitoring (Vanta, Drata, Secureframe, custom SOAR) need to verify the chain on a cron from a machine, pin the resulting `last_hash` to an external notary, WORM S3 Object Lock bucket, or git-anchored evidence manifest, and page on `ok=false` from the same SIEM that already ingests `/v1/audit`. The new endpoint walks every entry in the on-disk audit log in chronological order, verifies for each chained entry that `sha256(canonical(entry without hash)) == entry.hash` and that `entry.prevHash` matches the hash of the previous chained entry, returns 200 with `{ ok: true, total_entries, chained_entries, legacy_entries, first_day, last_day, last_hash, broken_at: null }` when the chain is intact, and 409 with `{ ok: false, ..., broken_at: { day, seq, id, reason } }` when any tamper is detected (`hash_mismatch`, `prev_hash_mismatch`, `seq_out_of_order`, `invalid_json`). The response also carries an `X-Audit-Chain-Status: ok|broken` header so SIEM correlation rules can match on the header alone without parsing the body. The chain is intentionally a single global sequence across every workspace (splitting it per tenant would let a workspace owner silently truncate their own slice, defeating the control), but cross-tenant isolation still holds because the endpoint never returns individual audit entries (use `GET /v1/audit` for that, which is workspace-scoped), the caller must hold a workspace-scoped Bearer key with `audit:read`, keys with no workspace receive 403, and the act of verifying is itself audited under the caller's workspace as `v1.audit.verify` so a tampering attempt cannot quietly verify. The standard `/v1` enforcement chain (lockdown, workspace + per-key IP allowlists, residency, API-key policy) runs before any disk read, the per-key rate-limit window is enforced (not peeked) so the verification call cannot be used as a free heartbeat against the dashboard process, and verification does not charge plan quota because it is metadata, not a billable model call. Pinned by `web/tests/v1-audit-verify-tenant-isolation.test.ts` (source-level wiring of scope, enforce-not-peek rate-limit, full workspace enforcement chain, no-workspace 403, stable `v1.audit.verify` action id and 200/409 status surface, plus a live test that cross-tenant verify rows attribute to the caller's workspace only and that flipping a byte in a stored entry causes `verifyAuditChain` to detect the break).

  ### Try it: pin the audit-chain head on a SOC2 evidence cron

  ```bash
  # Verify the immutable audit chain and capture the head hash for pinning.
  HEAD=$(curl -fsS https://codeclone.example.com/v1/audit/verify \
    -H "Authorization: Bearer $CODECLONE_KEY" \
    | tee /tmp/audit-verify.json \
    | jq -r .last_hash)

  # Anchor it externally (notary, WORM bucket, internal git evidence repo).
  printf '%s  %s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$HEAD" \
    >> ./evidence/codeclone-audit-head.log

  # On break, the route returns 409 + X-Audit-Chain-Status: broken so a
  # SIEM rule can page on either the status code or the header without
  # parsing the body. broken_at carries day, seq, id, and reason.
  ```
...
- Programmatic share-collection membership edits via `POST /v1/collections/{id}/items` and `DELETE /v1/collections/{id}/items?shareId=...`. The list/create/patch/delete verbs already shipped, but a curation bot that wanted to keep a collection fresh (drop the share that was retired, attach the one CI flagged this morning) still had to re-PUT the entire `shareIds` array through the cookie-only dashboard route, which lost ordering on concurrent edits and could not run from a release pipeline. The new verbs add or remove a single share by id behind the existing `collections:write` scope; no new scope to provision and no new dashboard surface to learn. Tenant scope is structural and runs on both sides of the link: the parent collection's `workspaceOwns` check returns a flat 404 on cross-workspace ids, and the share itself is loaded under `{ workspaceId: key.workspaceId }` via the new `addItem(..., { shareScope })` option, so a workspace A key cannot link a workspace B share into a workspace A collection and cannot use the membership-edit verb as a cross-tenant existence oracle (the response is 404 "share not found", indistinguishable from a typo). Every edit writes a `v1.collections.item_add` or `v1.collections.item_remove` row to the tamper-evident audit chain; the full `/v1` enforcement chain (lockdown, workspace + per-key IP allowlists, residency, API-key policy, DPA, per-key rate limit with `X-RateLimit-*` and `Retry-After`) runs before any storage write. Documented in `/v1/discovery`, `/v1/openapi.json|yaml`, and `/docs`. Pinned by `web/tests/v1-collection-items-tenant-isolation.test.ts` (live cross-tenant share-link denial at the lib layer plus source-level wiring checks that fail on regression if the scope, workspace gate, `shareScope` plumbing, audit, or rate-limit/IP/residency/lockdown enforcement are dropped).
  ...
  ```bash
  # Add a share CI flagged this morning to an existing collection:
  curl -sS -X POST http://localhost:3000/v1/collections/abc1234567/items \
    -H "Authorization: Bearer $CODECLONE_KEY" \
    -H "Content-Type: application/json" \
    -d '{"shareId":"def4567890"}' \
    | jq '.collection | {id, items: (.shareIds | length)}'

  # Drop a share that was retired:
  curl -sS -X DELETE "http://localhost:3000/v1/collections/abc1234567/items?shareId=def4567890" \
    -H "Authorization: Bearer $CODECLONE_KEY" | jq '.collection.shareIds'

  # A share owned by another workspace looks like a typo, never a leak:
  curl -sS -o /dev/null -w "%{http_code}\n" -X POST \
    http://localhost:3000/v1/collections/abc1234567/items \
    -H "Authorization: Bearer $CODECLONE_KEY" \
    -H "Content-Type: application/json" \
    -d '{"shareId":"otherWorkspaceShareId"}'
  ```

- Programmatic single webhook delivery fetch via `GET /v1/webhooks/{id}/deliveries/{deliveryId}`. The list endpoint at `/v1/webhooks/{id}/deliveries` was already programmatic, but support engineers, incident-response runbooks, and SDKs that wanted to confirm the outcome of a single replay still had to fetch and scan up to 200 rows from the list endpoint to find the one delivery id pasted into a ticket. The new verb returns one canonical delivery record (status, attempts, latency, request and response previews) behind the existing `webhooks:read` scope, with no new scope to provision and no new dashboard surface to learn. Tenant scope is structural: the route calls `loadWebhookForWorkspace(id, key.workspaceId)` and then filters `listDeliveriesForWorkspace(id, key.workspaceId)` by id, both of which refuse cross-tenant access at the lib layer, so an id from another workspace returns a flat 404 with no cross-tenant existence oracle; keys with no workspace binding receive `tenant_required` instead of falling through. The full `/v1` enforcement chain (lockdown, workspace IP allowlist, per-key IP allowlist, residency, API-key policy, rate limit) runs before any storage call. Pinned by `web/tests/v1-webhook-delivery-get-tenant-isolation.test.ts` (cross-tenant single-delivery lookup returns null at the lib level via a hermetic stub fetch, plus source-level wiring checks that fail on regression if the scope, workspace gate, usage logging, or rate-limit/IP/residency/lockdown enforcement are dropped).

- Programmatic webhook edit-in-place via `PATCH /v1/webhooks/{id}`. Until this change, a control plane that needed to repoint a webhook to a new URL after a vendor migration, narrow the subscribed event set as part of a least-privilege review, or pause delivery during an incident response had to delete the webhook and recreate it, which rotated the signing secret out from under every receiver and broke HMAC verification at exactly the wrong time. The new verb accepts the same Bearer (or `x-api-key`) credential as the rest of `/v1` behind the existing `webhooks:write` scope (no new scope to provision, no new dashboard surface to learn) and runs the full workspace enforcement chain (lockdown, workspace IP allowlist, per-key IP allowlist, residency, API-key policy, per-key rate limit) before touching webhook state. The signing secret is intentionally NOT mutable here so a URL change cannot collaterally invalidate the HMAC key receivers verify against; rotation keeps its dedicated dual-secret flow at `POST/PUT/DELETE /v1/webhooks/{id}/rotate`. Tenant scope is structural: every store call goes through `updateWebhook(id, key.workspaceId, ...)` which calls `webhookBelongsTo` and refuses cross-tenant ids at the lib layer with a flat 404 so the existence of another workspace's webhook id cannot be probed; keys with no workspace binding get `tenant_required` instead of falling through. URL changes re-run the workspace webhook domain allowlist check (so a customer cannot bypass the allowlist by creating against an allowed domain and then PATCHing to an arbitrary one), the http(s)-only + private-host guard, and the URL length cap. Event lists are sanitized against `SUPPORTED_EVENTS` (unknown events silently dropped, empty resolves to the safe default). Supports `dry_run=true` (query or body) which runs every auth/policy/quota check then returns the would-be summary without writing or recording usage. Every PATCH (real or dry-run) writes a `v1.webhooks.update` or `v1.webhooks.update.dry_run` row to the tamper-evident audit chain with a before/after diff of every changed field (url, events, label, disabled) so a SOC2 CC6.1 reviewer can reconstruct exactly which field changed when and from which key, satisfying configuration-change evidence requirements for integration endpoints. Documented in `/v1/discovery`, `/v1/openapi.json|yaml`, and the in-product `/docs` reference. Pinned by `web/tests/v1-webhooks-patch.test.ts` (lib-level proof that partial edits land without rotating the secret, that no-op edits don't mutate, that bad URLs and disallowed domains are rejected without touching the record, and that cross-tenant edits are refused without touching disk, plus source-level wiring checks that fail on regression if the scope, workspace gate, audit-with-diff, dry-run, or rate-limit/IP/residency/lockdown enforcement are dropped).

- Programmatic webhook test-ping via `POST /v1/webhooks/{id}/ping`. Customers integrating webhooks need to prove HMAC verification works in their CI before flipping a webhook live; until this change the only way to fire a signed `webhook.ping` was the cookie-authenticated dashboard button, which blocked SDK test suites, integration-validation jobs, and post-rotation smoke checks from acting without a human session. The endpoint takes the workspace from the calling Bearer key (no `workspaceId` query parameter to spoof), runs the full `/v1` enforcement chain (lockdown, workspace + per-key IP allowlist, residency, API-key policy, per-key rate limit), then re-uses the same `pingWebhook` lib path the dashboard uses, so a passing ping increments the same `successCount`, appends the same delivery-log row, and exercises the same HMAC signer as a live event. Returns 200 if the receiver answered 2xx, 502 otherwise (so a CI gate can branch on the HTTP status without parsing JSON), 409 with `webhook_disabled` if the webhook is paused, and a flat 404 for ids in other workspaces (no cross-tenant existence oracle). Sits behind the existing `webhooks:write` scope, no new scope to provision. Every attempt (success, failure, AND the disabled-webhook denial) writes a `v1.webhooks.ping` row to the tamper-evident audit chain with actor key id, target webhook id and label, workspace id, delivery id, receiver HTTP status, attempt count, and latency, so a SOC2 reviewer can pivot from any test ping back to which key fired it. Pinned by `web/tests/v1-webhooks-ping-tenant-isolation.test.ts` (cross-tenant ping denied at the lib level without bumping counters or appending a delivery row on the target webhook, plus source-level wiring checks that fail on regression if the scope, workspace gate, audit row, 200/502 status contract, or rate-limit/IP/residency/lockdown enforcement are dropped).
...
- Programmatic edit-in-place for workspace API keys via `PATCH /v1/keys/{id}`, so security teams can continuously narrow scopes, shrink the per-key rate-limit, tighten the per-key IP allowlist, shift the expiry deadline, or rename a key without rotating the secret and breaking every running pipeline. Closes a real procurement gap: SOC2 CC6.1 / ISO 27001 A.9.2 reviewers expect evidence that least-privilege is enforced on a continuous basis (not just at issuance), and the only previous remediation paths were full rotation (forces every consumer to ship a new secret) or revoke-and-recreate (causes a downtime window). The route takes the same Bearer token (or `x-api-key`) as the rest of `/v1`, requires the existing `keys:write` scope, and accepts any combination of `label`, `scopes` (narrowing only, never widening, so a PATCH can never accidentally grant privileges the key did not already hold), `rpm` (`null` clears the per-key override and falls back to the workspace default), `ipAllowlist` (CIDR list; `null` or `[]` clears it), and `expiresAt` (epoch ms; clamped down to the workspace API key max-age policy on write; `null` clears it unless the workspace policy demands an expiry, in which case the clear is refused). Tenant scope is structural via `updateKeyForWorkspace`: cross-workspace ids return 404 (not 403) so the existence of another tenant's key id cannot be inferred; keys with no workspace binding get 403 `tenant_required`. Self-target protection rejects a caller's attempt to PATCH the key it is currently authenticating with, so narrowing your own scopes mid-flight cannot brick the calling automation. Revoked or expired keys are refused with 400. The standard workspace enforcement chain (lockdown, workspace IP allowlist, per-key IP allowlist, residency, API-key policy) runs before any side effect, the per-key rate-limit window is enforced (not peeked), the dry-run preview path is honored via the standard `X-CodeClone-Dry-Run` header so a SOAR playbook can render the proposed change without committing, and every mutation (and every dry-run preview) writes a `v1.keys.update` / `v1.keys.update.dry_run` row to the tamper-evident audit chain with the actor key id, target key id, label, and a structured before/after diff covering label, scopes, rateLimit, ipAllowlist, and expiresAt for SOC2 CC6.1 change-evidence. Pinned by `web/tests/v1-keys-update-tenant-isolation.test.ts` (cross-tenant PATCH refusal, scope narrowing succeeds with a real diff, scope widening is refused, rpm/ipAllowlist set and clear round-trip, revoked keys cannot be edited, expiresAt validates a future timestamp, and a source-level wiring check that fails on regression if the `keys:write` scope filter, the self-target guard, the workspace-scoped store call, the audit row, or the dry-run preview path is dropped).
...
- MFA backup-code rotation at `/settings/security`: when an admin has lost their printed sheet of backup codes or has burned through too many of them, they can issue a fresh set of ten single-use codes without disabling MFA and re-enrolling (which would leave the account at single-factor for the duration of the rotation). The Regenerate backup codes panel asks for a current 6-digit authenticator code, calls `/api/auth/mfa/challenge` to mint a fresh step-up grant on the active session, then `POST /api/auth/mfa/backup-codes/regenerate` atomically replaces every unused hash on disk and returns the ten new plaintext codes exactly once. Old unused codes stop working the moment the rotation lands so a leaked sheet cannot be reused. Cookie session required, step-up required (a stolen cookie alone cannot silently rotate codes and lock the legitimate owner out), audited as `mfa.backup_codes.regenerate` with the count of codes the user discarded so SecOps can correlate the rotation against a stolen-laptop ticket. Pinned by `web/tests/mfa.test.ts` (replacement invalidates old codes, new codes are still single-use, rotation refuses for accounts without MFA enabled). Try it: open http://localhost:3000/settings/security, scroll to **Regenerate backup codes**, enter your current 6-digit code, **Regenerate codes**. `curl -i -b cc_session=... -X POST http://localhost:3000/api/auth/mfa/backup-codes/regenerate`.
