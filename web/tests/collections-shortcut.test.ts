// Collections page keyboard shortcut wiring.
// Pins the "/" focus-search shortcut on /collections so power users can jump
// to the search box without reaching for the mouse, matching the convention
// used by GitHub, Linear, and Slack (and the same shortcut already live on
// /history and /snippets). Source-level so it runs with the same node --test
// rig the rest of the suite uses, no jsdom required.
...
test("collections page binds a global / shortcut that focuses the search input", async () => {
  const src = await read("app/collections/page.tsx");
...
test("collections search box advertises the / shortcut", async () => {
  const src = await read("app/collections/page.tsx");
