// Audit page keyboard shortcut wiring.
// Pins the "/" focus-search shortcut on /audit so power users can jump to
// the action filter box without reaching for the mouse, matching the
// convention used by GitHub, Linear, and Slack (and the same shortcut
// already live on /history, /snippets, /collections, and /pairs).
...
test("audit page binds a global / shortcut that focuses the action input", async () => {
  const src = await read("app/audit/page.tsx");
...
test("audit page advertises the / shortcut", async () => {
  const src = await read("app/audit/page.tsx");
