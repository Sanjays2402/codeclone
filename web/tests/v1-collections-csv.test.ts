/**
 * Run with: node --test --experimental-strip-types web/tests/v1-collections-csv.test.ts
 *
 * Covers ?format=csv on GET /v1/collections. The route handler imports
 * next/server and cannot be loaded under raw `node --test`, so this
 * follows the existing source-level wiring pattern used by the other
 ...
 * type) without bypassing the enforcement chain. It also pins that
...
test("v1/collections: route wires format=csv and rejects unknown formats", () => {
  ...
  // they can tell apart from other workspaces' collection exports.
  assert.match(routeSrc, /text\/csv; charset=utf-8/);
  assert.match(routeSrc, /codeclone-\$\{filenameWs\}-collections\.csv/);
  // RFC 4180 double-quote escaping must live in the local CSV helper
  // so a collection title containing a comma or quote does not
  // corrupt the row alignment when imported into Excel.
  assert.match(routeSrc, /csvCell/);
  assert.match(routeSrc, /replace\(\/"\/g, '""'\)/);
  // CSV header must list every collection-summary field a reviewer
  // needs to reconcile a workspace's collection inventory, plus the
  // workspace_id stamp so multi-tenant exports stay traceable.
  for (const col of [
    "id",
    "workspace_id",
    "title",
...
test("v1/collections: CSV must not bypass the enforcement chain", () => {
  ...
  // (no separate, unscoped fetch path) and must require a workspace
  // binding like the JSON path.
  assert.match(routeSrc, /listCollections\(/);
...
test("v1/collections: api-spec advertises ?format=csv so discovery and openapi regenerate", () => {
  const idx = specSrc.indexOf('id: "collections-list"');
  assert.ok(idx > 0, "collections-list entry missing from api-spec");
