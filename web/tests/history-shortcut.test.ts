// History page keyboard shortcut wiring.
// Pins the "/" focus-search shortcut on /history so power users can jump to
// the search box without reaching for the mouse, matching the convention used
// by GitHub, Linear, and Slack. Source-level so it runs with the same
...
test("history page binds a global / shortcut that focuses the search input", async () => {
  const src = await read("app/history/page.tsx");
...
test("history search box advertises the / shortcut", async () => {
  const src = await read("app/history/page.tsx");
